...
class _ForestBackgroundPainter extends CustomPainter {
  ...
  void _drawMoon(Canvas canvas, double w, double h) {
    ...
    final glowPaint = Paint()
      ...
      ..color = const Color(0xFF8BA8C8).withOpacity( );
    canvas.drawCircle(Offset(moonX, moonY), pulseR * 2.5, glowPaint);

    // Mid glow
    glowPaint.color = const Color(0xFFB8D4E8).withOpacity( );
    ...
    // Moon craters (subtle)
    final craterPaint = Paint()..color = const Color(0xFF9AB4CC).withOpacity( );
    ...
    for (int layer = 0; layer < 2; layer++) {
      ...
      final fogPaint = Paint()
        ..shader = LinearGradient(
          ...
          colors: [
            Colors.transparent,
            const Color(0xFF2A3A5A).withOpacity( ),
            const Color(0xFF3A4A6A).withOpacity( ),
            const Color(0xFF2A3A5A).withOpacity( ),
            Colors.transparent,
          ],
  ...
  void _drawFireflies(Canvas canvas, double w, double h) {
    for (final ff in fireflies) {
      ...
      // Glow
      final glowPaint = Paint()
        ..color = const Color(0xFFFFE082).withOpacity( )
