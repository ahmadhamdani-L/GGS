...
class _ProfilePageState extends ConsumerState<ProfilePage> {
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Container(
                  ...
                  decoration: BoxDecoration(borderRadius: BorderRadius.circular(6), gradient: AppColors.primaryGradient),
                  child: Text('Lv.${profile?.level ?? 1}', style: const TextStyle(color: AppColors.background, fontSize: 11, fontWeight: FontWeight.w700)),
                ),
                const SizedBox(width: 8),
  ...
  Widget _buildStatsRow() {
    ...
    return Row(children: [
      ...
      const SizedBox(width: 10),
      _miniStat('Rating', '$rating', AppColors.primary),
    ]);
  }
  ...
  Widget _buildXPProgress(dynamic profile) {
    ...
    // Simple progress calculation (100 XP per level approximately)
    final xpForCurrentLevel = level > 1 ? (level - 1) * 100 + ((level - 1) * 50) : 0;
    final xpForNextLevel = level * 100 + (level * 50);
    final progress = xpForNextLevel > xpForCurrentLevel
        ? ((xp - xpForCurrentLevel) / (xpForNextLevel - xpForCurrentLevel)).clamp(0.0, 1.0)
        : 0.0;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Spacer(),
          Text('$xp / $xpForNextLevel XP', style: const TextStyle(color: AppColors.textMuted, fontSize: 11)),
        ]),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Wrap(spacing: 10, runSpacing: 10, children: achievements.map((a) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            color: unlocked ? AppColors.primary.withValues(alpha: 0.06) : Colors.white.withValues(alpha: 0.02),
            borderRadius: BorderRadius.circular(12),
            border: Border.all(color: unlocked ? AppColors.primary.withValues(alpha: 0.2) : Colors.white.withValues(alpha: 0.05)),
          ),
          child: Row(children: [
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        GestureDetector(
          ...
          child: const Text('Lihat semua →', style: TextStyle(color: AppColors.primary, fontSize: 12, fontWeight: FontWeight.w600)),
        ),
      ]),
      const SizedBox(height: 12),
        ...
        ..._recentMatches!.take(5).map((m) {
          ...
          return Container(
            ...
            child: Row(children: [
              ...
              const Spacer(),
              Text('+${match['xpEarned'] ?? 0} XP', style: const TextStyle(color: AppColors.primary, fontSize: 11, fontWeight: FontWeight.w600)),
            ]),
          );
        }),
    ]);
